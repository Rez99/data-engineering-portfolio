...
| **M4** | Clean Kafka → Flink → Postgres | Compute live session-level bot detection metrics using stateful stream processing and continuously update an operational PostgreSQL database. | 🟡 In progress |
...
| M4        | `setup_m4.sh`  | `reset_m4.sh`  | 🟡 In progress |
...
M4.1/M4.2 verification confirmed Docker Compose can run DuckDB, DuckDB can query the M3 partitioned Parquet dataset, the source dataset contains 42,448,762 rows across 31 `event_date` partitions and 658 Parquet part files, `normalization.parquet` contains 101 percentile rows, and `bot_config.json` contains a 99th-percentile session inactivity timeout of 5,220,000 ms.
...
| ID | Task | Acceptance Criteria | Status |
|---|---|---|---|
| **M6.1** | Deploy monitoring tools | - Redpanda Console and the Flink Web UI are running locally.<br>- Both tools are accessible from the development environment. | 🔴 Not started |
